...
def dashboard(request):
    ...
    fb_links = FBVideoLinks.objects.filter(customer=customer)
    tw_links = TWVideoLinks.objects.filter(customer=customer)
    no_of_videos=videos.count()
    context = {'videos':videos,'no':no_of_videos,'links':links,'fb_links':fb_links,'tw_links':tw_links,'customer':customer}
...
def home(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            video.customer = request.user.customer 
...
def audio_page(request, audio_fileName):
    ...
    context = {'video': video, 'form': form, 'comments': comments,'customer':customer}
...
def audio_page_links(request , audio_fileName):
    ...
    context = {'links':links,'comments':comments,'form':form,'customer':customer}
    return render(request, 'accounts/audio_page_links.html',context)
    
@login_required(login_url='login')
def fb_audio_page_links(request , audio_fileName):
    links = get_object_or_404(FBVideoLinks, audio_fileName=audio_fileName)
    customer = request.user.customer
    if request.method == "POST":
        form = FBCommentLinkForm(request.POST)
        if form.is_valid():
            comment = form.save(commit=False)
            comment.customer = request.user.customer
            comment.fb_link = links 
            comment.save()
            return redirect(request.path)
    else:
        form = FBCommentLinkForm()
    comments = FBCommentLinks.objects.filter(fb_link=links)
    context = {'links':links,'comments':comments,'form':form,'customer':customer}
    return render(request, 'accounts/fb_audio_page_links.html',context)
    
@login_required(login_url='login')
def tw_audio_page_links(request , audio_fileName):
    links = get_object_or_404(TWVideoLinks, audio_fileName=audio_fileName)
    customer = request.user.customer
    if request.method == "POST":
        form = TWCommentLinkForm(request.POST)
        if form.is_valid():
            comment = form.save(commit=False)
            comment.customer = request.user.customer
            comment.tw_link = links 
            comment.save()
            return redirect(request.path)
    else:
        form = TWCommentLinkForm()
    comments = TWCommentLinks.objects.filter(tw_link=links)
    context = {'links':links,'comments':comments,'form':form,'customer':customer}
    return render(request, 'accounts/tw_audio_page_links.html',context)
...
def extract_audio_from_facebook(url):
    ...
    try:
        ...
        return True , f'{output_dir}/{fileid}.mp3',fileid
...
def mutagen_length(path):
    try:
        ...
        return None
@login_required(login_url='login')
def upload_facebook_video(request):
    customer = request.user.customer
    if request.method == 'POST':
        form = FBLinkUploadForm(request.POST)
        if form.is_valid():
            link = form.save(commit=False)
            video_link = form.cleaned_data['video_link']
            title = form.cleaned_data['title']
            customer = request.user.customer  # Retrieve the customer model of the current user
            success , audiofilepath,fileid = extract_audio_from_facebook(video_link)
            if success:
                link.audio_file.save(audiofilepath.split('/')[-1],open(audiofilepath, 'rb'))
                link.title = title
                project_folder = os.getcwd()
                audio_file_path = os.path.join(project_folder,'audios',f'{audiofilepath.split("/")[-1]}')
                duration = mutagen_length(audio_file_path)
                duration_formatted = format_duration(int(duration))
                link.duration = duration_formatted
                link.customer=customer
                link.audio_fileName=fileid
                link.save()
                os.remove(audiofilepath)
                return redirect('video_list')
    else:
        form = FBLinkUploadForm()
    return render(request, 'accounts/upload_facebook_video.html', {'form': form,'customer':customer})

@login_required(login_url='login')
def upload_twitch_video(request):
    customer = request.user.customer
    if request.method == 'POST':
        form = TWLinkUploadForm(request.POST)
        if form.is_valid():
            link = form.save(commit=False)
            video_link = form.cleaned_data['video_link']
            title = form.cleaned_data['title']
        ...
        form = FBLinkUploadForm()
    return render(request, 'accounts/upload_twitch_video.html', {'form': form,'customer':customer})